...
## Explanation of code
Moving 40 objects on the screen in Atari BASIC with some manner of speed? Impossible! So how did I do it?  Custom charsets to the rescue!  Instead of constantly redrawing the snowflakes, the screen stays mostly static, and I blank out nearly the entire font, then move a snowflake character through the characters, one byte (or row of pixels in a single character) at a time.

#### Custom charset to the rescue!

Here is what the screen looks like when the custom charset is not enabled.
<img src="snow-charset.png">

It is a bunch of static columns of ascending characters. e.g. "ABCD".

For example, consider the following characters that would normally display onscreen like this (Z wraps back to A):
```
-----
A   X
B   Y
...
#### Smooth scrolling
In order to get the snowflakes to fall pixel by pixel, I take advantage of the fact that an Atari charset is laid out in memory as a series of vertical rows. A character is represented by 8 rows of 8 pixels each. Each row is represented by a byte, so 8 bytes a character.

![A Atari Charset character](charset-char.png)

* (Image taken from https://www.atariarchives.org/agagd/chapter3.php - go here to learn more about Atari charsets).

When I shift the character from A to B by 2 bytes, some rows of the star are at the bottom of "A" and 2 are at the top of "B". Since I drew "B" directly below "A" onscreen, the snowflake appears to smoothly fall down.

![Snow character movement example](snow-memory.png)

In order to have 3 different tracks (and sizes) of snowflakes, I use three sets of contiguous characters characters in the character set to do this.

Finally, I use a similar trick for the growing snow at the bottom. I create 8 characters, each character a taller block, then update the screen one character per frame to show the snow accumulating, until I overwrite the entire screen.

#### Atari Charset

Here is the Atari charset, shown in rows of 32 characters each:
![Character set groups](charset-groups.png)

I divided the 128 characters into 4 rows like above. The first 3 rows are used for the 3 types of snowflakes that fall. The final group is used for the growing snow, and "let it snow" text on the screen.

If you look at the static screen again, you can match up characters in each row to the columns shown on the screen.


## Explanation of code

To make this slightly less confusing, I have re-aranged some of the 10-liner statements in a more logical way to understand. It is the same code, just split into more lines.
...
Here I loop three times and copy the 3 characters (3 sets of characters * 8 bytes each) from the DATA statement into the B$ string buffer. I copy the character at the start and the end (+192) of each buffer, so when I copy a full set of 24 characters (24*8=192), it will copy the beginning or end of the character as well, instead of cutting off part of the character when it flips over the boundary of, say, Z and goes back to A. In other words, to prevent a situation where  Z (at the end in memory) would have the first few rows of the snowflake, but A (start of memory) would not.
...
This way, I always get the proper bits of the character at the ends, which will happen when rolling over from the last character in memory back to the first character, starting the cycle again.
```
