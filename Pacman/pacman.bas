...
lives=0:mge1=0:mge2=0:mgi=0:quick_start=0:h_c=0:h1=0:h2=0
...
if subf2=10
  subf2=0
  if key():get k:repeat:until key():get K:e.
...
' End of 60 frames - increase enemy state counter and do other checks
  ...
  if es_c>99
    ...
    if died=1 
      ...
      es_c=0
      ' Revert enemies to normal color/anim
      if es=E_AVOID: for i=1 to 4:@anim ad(i)+7:n.: @colorNormal:gm=l_gm(level): e.
      inc es_i
      es=E_ROAM
    e.
  e.
...
PR.newGame
  level=-1:score=0:lives=3:f_c=0:noLifeLost=0:ISI=0:skipSong=0
  @nextLevel
ENDP.
...
if skipSong then exit
NL=0:NH=0:hvol=8
for i = ISI  to ISI+31
if I>ISI t. pause 3
if SL(i)>0 :NL=N(SL(i)):vol=9: elif vol>0 :vol=vol-3: e.:SOUND 0,NL,14,vol
IF SH(i)>0 :NH=N(SH(i)):hvol=8: else: hvol=0: e.:SOUND 1,NH,14,hvol
n.
ISI=i
pause
if ISI>60 
for i= 6 to 0 step -1:sound 1,NH,14,I:pause:n.:sound 0,0,0,0
skipSong=1
e.
ENDP.
